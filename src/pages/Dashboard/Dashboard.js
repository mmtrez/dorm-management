  ...
  return (
    <Box flexGrow={1} paddingY={6} paddingX={3}>
            ...
            reports={
              role === "user"
                ? reports
                    .filter(
                      (report) =>
                        report.reporter === userInfo.fullName &&
                        report.reportState !== "محول شده"
                    )
                    .reverse()
                : role === "worker"
                ? reports
                    .filter(
                      (report) =>
                        report.handler === userInfo.fullName &&
                        report.handlerAnswer === "پذیرفته شده"
                    )
                    .reverse()
                : reports
                    .filter((report) => report.reportState === "محول شده")
                    .reverse()
            }
            ...
            reports={
              role === "user"
                ...
                ? reports.filter(
                    (report) =>
                      report.handler === userInfo.fullName &&
                      report.handlerAnswer === "بدون پاسخ"
                  )
                : reports
                    .filter((report) => report.reportState === "بررسی نشده")
                    .reverse()
            }
            setReports={setReports}
            newWorker={newWorker}
      ...
      {role !== "worker" && (
        <Stack
          direction="row"
          sx={{ position: "fixed", left: "1.5rem", bottom: "1.5rem" }}
        >
          <Fab
            variant="extended"
            color="primary"
